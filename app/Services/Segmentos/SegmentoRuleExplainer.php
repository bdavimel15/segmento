...
class SegmentoRuleExplainer
{
    ...
    private function resolveFieldValue(array $row, string $field): mixed
    {
        ...
        return match ($field) {
            ...
            'qtd_pedidos', 'qtd_pedidos_confirmados' => $row['_seg_qtd_pedidos'] ?? $row['_seg_qtd_pedidos_confirmados'] ?? $row['cli_qtd_pedidos_calc'] ?? $row['cli_qtd_pedidos'] ?? null,
            'ultimo_pedido', 'ultima_compra' => $row['_seg_ultimo_pedido'] ?? $row['_seg_ultima_compra'] ?? $row['cli_ultimo_pedido_calc'] ?? null,
            'primeira_compra' => $row['_seg_primeira_compra'] ?? $row['cli_primeira_compra_calc'] ?? null,
            'valor_total_comprado', 'valor_total_compras' => $row['_seg_valor_total_comprado'] ?? $row['_seg_valor_total_compras'] ?? $row['cli_valor_total_comprado_calc'] ?? null,
            'cashback', 'cashback_saldo' => $row['_seg_cashback'] ?? $row['_seg_cashback_saldo'] ?? $row['cli_cashback_calc'] ?? null,
            ...
            'produto_comprado', 'produto_nome', 'produto' => $row['_seg_produto_comprado'] ?? $row['_seg_produto_nome'] ?? $row['_seg_produto'] ?? $row['cli_produtos_comprados_calc'] ?? null,

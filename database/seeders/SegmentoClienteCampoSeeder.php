...
class SegmentoClienteCampoSeeder extends Seeder
{
    public function run(): void
    {
        $campos = [
            ...
            ['chave'=>'sexo','label'=>'Sexo','descricao'=>'Sexo do cliente. Use Masculino ou Feminino.','categoria'=>'cliente','tipo_valor'=>'string','expressao_sql'=>"CASE WHEN c.sexo_id IN ('M','Masculino','masculino') THEN 'Masculino' WHEN c.sexo_id IN ('F','Feminino','feminino') THEN 'Feminino' ELSE c.sexo_id END",'operadores_json'=>['equals','not_equals','is_empty','is_not_empty'],'ordem'=>3],
            ...
            ['chave'=>'funcionario','label'=>'Funcionário(a)','descricao'=>'Indica se o cliente é funcionário.','categoria'=>'cliente','tipo_valor'=>'boolean','expressao_sql'=>"CASE WHEN c.cli_funcionario IN ('S','SIM','Sim','sim','1') THEN 'SIM' ELSE 'NÃO' END",'operadores_json'=>['is_true','is_false','equals','not_equals'],'ordem'=>10],
            ['chave'=>'newsletter','label'=>'Newsletter','descricao'=>'Indica se o cliente aceita receber comunicações.','categoria'=>'cliente','tipo_valor'=>'boolean','expressao_sql'=>"CASE WHEN c.cli_newsletter IN ('S','SIM','Sim','sim','1') THEN 'SIM' ELSE 'NÃO' END",'operadores_json'=>['is_true','is_false','equals','not_equals'],'ordem'=>11],
            ...
            ['chave'=>'produto_comprado','label'=>'Produto comprado','descricao'=>'Produto comprado em algum pedido confirmado. Ex.: picanha, pizza, hambúrguer.','categoria'=>'produto','tipo_valor'=>'string','expressao_sql'=>'prod.produtos_comprados','operadores_json'=>['equals','not_equals','contains','not_contains','starts_with','ends_with','is_empty','is_not_empty'],'ordem'=>20],
        ...
        foreach ($campos as $campo) {
            SegmentoClienteCampo::updateOrCreate(['chave' => $campo['chave']], $campo + ['ativo' => 'S']);
        }
